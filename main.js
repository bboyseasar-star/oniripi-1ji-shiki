// main.js ── 1次関数の式マスター（複数入力フィールド対応・汎用版）
'use strict';
(function(){

/* ===== localStorage ===== */
const STORE_KEY = 'oniripi_shiki_v1';
const mem = {};
const store = {
...
function parseLinear(latex){
  ...
  const eq=s.indexOf('=');
  if(eq>=0) s=s.slice(eq+1);                       // 右辺だけにする
...
function loadQuestion(){
  ...
  // メモは問題切替で消去＋クイズ画面表示後にcanvas解像度を取り直す
...
function showResult(){
  ...
  const msgs=pass?['🏆 おにリピ クリア！','🔥 完ぺき！式マスター！','⭐ すごい！この調子！']
...
function initMemo(){
  ...
  // クイズ表示時・問題切替時にサイズを取り直す（flex確定後に解像度を合わせる）
